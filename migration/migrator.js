...
const migrate = async () => {
    ...
    let addOwnerRole = async () => {
        let dbOwnerUser = await userDB.findOne({ phone: "09100100100" });
        let dbOwnerRole = await roleDB.findOne({ name: "Owner" });
        if (dbOwnerUser && dbOwnerRole) {
            await userDB.findByIdAndUpdate(dbOwnerUser._id, { $push: { roles: dbOwnerRole._id } });
